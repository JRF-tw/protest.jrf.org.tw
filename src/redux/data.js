...
export const answerData = [
  {
    ...
    calculate: function calculate(state) { return ( state.a || state.b || state.c || state.o ) ? true : false; },
    key: 'criminal135'
  }, {
    answer: '刑法§136條，聚眾妨礙公務罪。',
    detail: '公然聚眾犯前條之罪者，在場助勢之人，處一年以下有期徒刑、拘役或三百元以下罰金。首謀及下手實施強暴脅迫者，處一年以上、七年以下有期徒刑。',
    calculate: function calculate(state) { return (( state.a || state.b || state.c || state.o ) && ( state.f || state.g )) ? true : false; },
    key: 'criminal136'
  }, {
    answer: '刑法§140條侮辱公務人員、侮辱公署罪',
    detail: '於公務員依法執行職務時，當場侮辱，或對於其依法執行之職務公然侮辱者，處六月以下有期徒刑、拘役或一百元以下罰金。<br />對於公署公然侮辱者亦同。',
    calculate: function calculate(state) { return ( state.b || state.d || state.e ) ? true : false; },
    key: 'criminal140'
  }, {
    answer: '刑法§153煽惑他人犯罪',
    detail: '以文字、圖畫、演說或他法，公然為左列行為之一者，處二年以下有期徒刑、拘役或一千元以下罰金：<br />一、煽惑他人犯罪者。<br />二、煽惑他人違背法令，或抗拒合法之命令者。',
    calculate: function calculate(state) { return ( state.f || state.g || state.k ) ? true : false; },
    key: 'criminal153'
  }, {
    answer: '刑法§306侵入建物罪',
    detail: '無故侵入他人住宅、建築物或附連圍繞之土地或船艦者，處一年以下有期徒刑、拘役或三百元以下罰金。<br />無故隱匿其內，或受退去之要求而仍留滯者，亦同。',
    calculate: function calculate(state) { return ( state.h || state.i ) ? true : false; },
    key: 'criminal306'
  }, {
    answer: '刑法§354損毀罪',
    detail: '毀棄、損壞前二條以外之他人之物或致令不堪用，足以生損害於公眾或他人者，處二年以下有期徒刑、拘役或五百元以下罰金。',
    calculate: function calculate(state) { return ( state.j ) ? true : false; },
    key: 'criminal354'
  }, {
    answer: '集遊法§29集會遊行不解散之首謀',
    detail: '集會、遊行經該管主管機關命令解散而不解散，仍繼續舉行經制止而不遵從，首謀者處二年以下有期徒刑或拘役。',
    calculate: function calculate(state) { return (( state.f || state.g ) && state.l ) ? true : false; },
    key: 'assembly29'
  }, {
    answer: '集遊法§30集會遊行侮辱公署罪',
    detail: '集會﹑遊行時，以文字﹑圖畫﹑演說或他法，侮辱﹑誹謗公署﹑依法執行職務之公務員或他人者，處二年以下有期徒刑﹑拘役或科或併科新台幣六萬元以下罰金。',
    calculate: function calculate(state) { return (( state.d || state.e ) && ( state.f || state.g )) ? true : false; },
    key: 'assembly30'
  }, {
    answer: '刑法§304強制罪',
    detail: '以強暴、脅迫使人行無義務之事或妨害人行使權利者，處三年以下有期徒刑、拘役或三百元以下罰金。<br />前項之未遂犯罰之。',
    calculate: function calculate(state) { return ( state.m || state.n ) ? true : false; },
    key: 'criminal304'
  }, {
    ...
    calculate: function calculate(state) { return ( state.n ) ? true : false; },
    key: 'criminal185'
  }
];
